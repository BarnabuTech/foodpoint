...
import { setSearchItems, setUserData, setSelectedCity } from "../redux/userSlice";
...
function Nav() {
  ...
  const availableCities = ["Mombasa", "Nairobi", "Kisumu", "Nakuru", "Eldoret"];

  // Set default selected city on mount if not set
  useEffect(() => {
    if (!selectedCity && currentCity) {
      dispatch(setSelectedCity(currentCity));
    }
  }, [currentCity, selectedCity, dispatch]);
  ...
  useEffect(() => {
    ...
    if (showInfo || showLocationDropdown) {
  ...
  }, [showInfo, showLocationDropdown]);

  const handelSearchItems = async () => {
    try {
      const result = await axios.get(
        `${serverUrl}/api/item/search-items?query=${query}&city=${selectedCity || currentCity}`,
    ...
    <div className="w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-gradient-to-r from-orange-50 via-red-50 to-pink-50 backdrop-blur-xl border-b border-white/20 shadow-lg overflow-visible">
      ...
      {showSearch && userData.role == "user" && (
        <div className="w-[90%] h-[70px] bg-white/90 backdrop-blur-lg shadow-2xl rounded-2xl items-center gap-[20px] flex fixed top-[90px] left-[5%] md:hidden transform animate-slideDown border border-white/20">
          <div className="flex items-center w-[30%] overflow-hidden gap-[10px] px-[15px] border-r-[2px] border-[#ff4d2d]/30 cursor-pointer" 
            onClick={() => setShowLocationDropdown(true)}
            ref={locationDropdownRef}>
            <div className="p-2 bg-[#ff4d2d]/10 rounded-full">
              <FaLocationDot size={16} className="text-[#ff4d2d]" />
            </div>
            <div className="w-[70%] truncate text-gray-700 font-medium text-sm flex items-center gap-1">
              {selectedCity || currentCity}
              <FaChevronDown size={10} className="text-gray-500" />
            </div>
            {/* Location Dropdown */}
            {showLocationDropdown && (
              <div className="absolute top-[70px] left-0 w-[200px] bg-white shadow-2xl rounded-2xl overflow-hidden z-50 border border-gray-200">
                ...
                {availableCities.map((city) => (
                  <div
                    key={city}
                    className={`px-4 py-3 cursor-pointer hover:bg-[#ff4d2d]/10 transition-colors ${
                      (selectedCity || currentCity) === city ? 'bg-[#ff4d2d]/20 text-[#ff4d2d] font-semibold' : 'text-gray-700'
                    }`}
      ...
      {/* Desktop Search Bar */}
      {userData.role == "user" && (
        <div className="md:w-[60%] lg:w-[50%] h-[60px] bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl items-center gap-[20px] hidden md:flex border border-white/20 hover:shadow-2xl transition-all duration-300 group relative">
          <div className="flex items-center w-[35%] overflow-hidden gap-[12px] px-[20px] border-r-[2px] border-[#ff4d2d]/20 cursor-pointer relative"
            ...
            <div className="w-[70%] truncate text-gray-700 font-medium flex items-center gap-2">
              {selectedCity || currentCity}
              <FaChevronDown size={12} className="text-gray-500" />
            </div>
            {/* Location Dropdown for Desktop */}
            {showLocationDropdown && (
              <div className="absolute top-[65px] left-[10px] w-[200px] bg-white shadow-2xl rounded-2xl overflow-hidden z-50 border border-gray-200">
                <div className="p-3 bg-gray-50 border-b border-gray-200">
                  <p className="text-xs font-semibold text-gray-500 uppercase">Select Location</p>
                </div>
                {availableCities.map((city) => (
