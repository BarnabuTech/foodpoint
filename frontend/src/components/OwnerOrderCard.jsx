...
const OwnerOrderCard = ({ data }) => {
  ...
  if (!data || !data.shopOrders || data.shopOrders.length === 0) {
    return (
      <div className="bg-linear-to-br from-gray-50 to-gray-100 rounded-2xl shadow-lg p-8 border border-gray-200">
  ...
  return (
    <div className="bg-linear-to-br from-white via-orange-50/30 to-pink-50/20 rounded-3xl shadow-2xl border border-orange-100 overflow-hidden">
      {/* Header Section with Gradient */}
      <div className="bg-linear-to-r from-orange-500 to-red-500 p-6 text-white relative overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        {/* Payment Info Card */}
        <div className="bg-linear-to-br from-green-50 to-emerald-50 rounded-2xl p-4 border-2 border-green-200 shadow-sm">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-linear-to-br from-blue-50 to-indigo-50 rounded-2xl p-4 border-2 border-blue-200 shadow-sm">
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 bg-blue-500 rounded-xl flex items-center justify-center shadow-lg shrink-0">
        ...
        {/* Order Items Section */}
        <div className="bg-linear-to-br from-orange-50 to-red-50 rounded-2xl p-5 border-2 border-orange-200 shadow-sm">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">
                      <span className="inline text-xs font-bold">Ksh</span>
                      {item?.item?.price || "NA"} × {item.quantity}
                    </span>
                    <span className="text-lg font-bold text-orange-600">
                      <span className="inline text-sm font-bold">Ksh</span>
                      {item?.item?.price || "NA" * item.quantity}
                    </span>
                  </div>
                </div>
        ...
        <div className="bg-linear-to-br from-purple-50 to-pink-50 rounded-2xl p-5 border-2 border-purple-200 shadow-sm">
                ...
                <span
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold mt-1 shadow-sm ${
                    shopOrder?.status === "pending"
                      ? "bg-linear-to-r from-yellow-400 to-yellow-500 text-yellow-900"
                      : shopOrder?.status === "preparing"
                      ? "bg-linear-to-r from-blue-400 to-blue-500 text-blue-900"
                      : shopOrder?.status === "out of delivery"
                      ? "bg-linear-to-r from-orange-400 to-orange-500 text-orange-900"
                      : "bg-linear-to-r from-green-400 to-green-500 text-green-900"
                  }`}
                >
                  {shopOrder?.status === "pending" && "⏳"}
                  {shopOrder?.status === "preparing" && "👨‍🍳"}
        ...
        {shopOrder?.status === "preparing" && (
          <div className="bg-linear-to-br from-indigo-50 to-purple-50 rounded-2xl p-5 border-2 border-indigo-200 shadow-sm">
            ...
            {availableBoys && availableBoys.length > 0 ? (
              <div className="space-y-3">
                {availableBoys.map((b, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-linear-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center text-white font-bold shadow-md">
              ...
              <div className="bg-white rounded-xl p-4 border-2 border-green-200 shadow-sm">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-linear-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg">
        ...
        {/* Total Section */}
        <div className="bg-linear-to-r from-orange-500 to-red-500 rounded-2xl p-6 shadow-2xl">
