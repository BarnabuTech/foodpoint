...
// M-Pesa Logo SVG (Embedded - will always display)
const MpesaIcon = () => (
  <svg viewBox="0 0 140 50" className="h-12 w-auto">
    {/* Phone outline */}
    <rect x="5" y="5" width="30" height="40" rx="5" fill="#E8E8E8" stroke="#ccc" strokeWidth="1"/>
    {/* Phone screen (black) */}
    <rect x="8" y="10" width="24" height="26" rx="2" fill="#1a1a1a"/>
    {/* Home button */}
    <circle cx="20" cy="40" r="2.5" fill="#666"/>
    
    {/* Red checkmark on phone */}
    <path d="M12 22 L16 26 L26 16" stroke="#DC143C" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
    
    {/* Separator */}
    <line x1="42" y1="10" x2="42" y2="40" stroke="#ddd" strokeWidth="1"/>
    
    {/* M-PESA Text - Green */}
    <text x="50" y="35" fill="#00A650" fontSize="24" fontWeight="bold" fontFamily="Arial, sans-serif" letterSpacing="-1">M</text>
    <text x="72" y="35" fill="#00A650" fontSize="20" fontFamily="Arial, sans-serif">-</text>
    <text x="82" y="35" fill="#00A650" fontSize="20" fontWeight="bold" fontFamily="Arial, sans-serif" letterSpacing="1">PESA</text>
  </svg>
);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const result = await axios.post(
        ...
        {
          paymentMethod: paymentMethod === "mpesa" ? "online" : paymentMethod,
      ...
      if (paymentMethod === "cod") {
        ...
        alert("M-Pesa STK Push sent to your phone. Please complete the payment on your phone.");
        // Poll for payment status or wait for socket event
        setTimeout(() => {
          dispatch(clearCart());
          navigate("/order-placed");
        }, 3000);
      } else {
        // Fallback for old online payment (Razorpay) - shouldn't happen now
      ...
      <div className="w-full max-w-[900px] bg-white p-6 rounded-2xl shadow-xl space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* M-Pesa */}
            <div
              className={`cursor-pointer flex items-center justify-center rounded-xl border p-4 transition ${
                paymentMethod === "mpesa"
                  ? "border-[#ff4d2d] bg-orange-100 shadow"
                  : "border-gray-500 bg-white hover:shadow-md hover:border-gray-400"
              }`}
        ...
        <section>
          ...
          <div className="rounded-xl border bg-gray-50 p-4 space-y-3">
            ...
            <div className="flex justify-between text-lg font-bold text-[#ff4d2d] pt-2">
              <span>Total</span>
              <span>Ksh {amountWithDeliveryFee}</span>
            </div>
          </div>
        </section>

        <button
          className="w-full bg-[#ff4d2d] hover:bg-[#e64526] text-white py-3 rounded-xl font-semibold cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
